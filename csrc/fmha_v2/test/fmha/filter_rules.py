...
def host_softmax_limitation_rule(fmha_arg, **kwargs):
    if fmha_arg.seq_len not in [
        ...
        print(fmha_arg)
        pytest.skip(
            reason=f"host_softmax_limitation_rule: Host reference softmax only works for seq_len"
...
def flash_attn_limitation_rule(fmha_arg, **kwargs):
    ...
    if fmha_arg.seq_len >= 1024 and fmha_arg.precision == "int8":
        pytest.skip(
            reason=f"flash_attn_limitation_rule: int8 does not yet support seq_len>=1024. "
            "{fmha_arg}"
        )
    if cc_tag == 70 and (fmha_arg.head_dim > 128 or fmha_arg.head_dim <= 16):
        pytest.skip(
            reason=f"flash_attn_limitation_rule {cc_tag}: head_dim not supported {fmha_arg}"
        )
    ...
    if cc_tag == 89 and (fmha_arg.head_dim > 256):
        pytest.skip(
            reason=f"flash_attn_limitation_rule {cc_tag}: head_dim not supported due to SMEM"
            " size. {fmha_arg}"
        )
...
def non_flash_attn_limitation_rule(fmha_arg, **kwargs):
    ...
    if cc_tag < 80 and fmha_arg.seq_len == 512:
        pytest.skip(
            reason=f"non_flash_attn_limitation_rule: seq_len not supported due to SMEM size. "
            "{fmha_arg}"
        )
    if cc_tag == 75 and fmha_arg.seq_len == 96:
        pytest.skip(
            reason=f"non_flash_attn_limitation_rule: seq_len not implemented in arch. {fmha_arg}"
        )
    if fmha_arg.seq_len > 512:
        pytest.skip(
            reason=f"non_flash_attn_limitation_rule: does not support seq_len > 512. "
            "{fmha_arg}"
        )
    if fmha_arg.head_dim not in [16, 32, 64]:
        pytest.skip(
            reason=f"non_flash_attn_limitation_rule: does not support head_dim other than 16, 32, 64. "
            "{fmha_arg}"
        )
...
def corner_case_rule(fmha_arg, **kwargs):
    ...
    if [fmha_arg.seq_len, fmha_arg.head_dim] in corner_cases:
        pytest.skip(
            reason=f"corner_case_rule: seq_len/head_dim combo not supported by kernels. {fmha_arg}"
        )
    if fmha_arg.seq_len == 32 and fmha_arg.precision == "int8":
        pytest.skip(
            reason=f"corner_case_rule: seq_len/precision combo not supported by kernels. {fmha_arg}"
        )
    if fmha_arg.head_dim > 64 and fmha_arg.precision == "int8":
        pytest.skip(
            reason=f"corner_case_rule: head_dim/precision combo not supported by kernels. {fmha_arg}"
        )
...
def avoid_oom_rule(fmha_arg, **kwargs):
    ...
    h = fmha_arg.num_head
    d = fmha_arg.head_dim
    if max(bytes_per_elt * s * s * b * h, 4 * bytes_per_elt * b * s * h * d) > (
        4 * 1024 * 1024 * 1024
    ):
        pytest.skip(
            reason=f"avoid_oom_rule: QKVO matrices or P matrix exceed 4 GiB. {fmha_arg}"
        )
...
def grouped_query_attention(fmha_arg, **kwargs):
    assert (
        fmha_arg.num_head % 2 == 0
    )  # assert equal number of heads across query groups
...
def _non_sm90_rule(fmha_arg):
    ...
    if fmha_arg.force_non_flash_attention and (
        ...
        pytest.skip(
            reason=f"rule: head_dim/seq_len is not supported for non-flash-attention kernels. {fmha_arg}"
        )
...
def _sm70_rule(fmha_arg):
    if fmha_arg.precision in ["int8", "e4m3", "bf16", "fp16-fp32"]:
        pytest.skip(
            reason=f"sm70_rule: int8/fp8/bf16/fp16-fp32 not supported. {fmha_arg}"
        )
...
def _sm75_rule(fmha_arg):
    if fmha_arg.precision in ["int8", "e4m3", "bf16", "fp16-fp32"]:
        pytest.skip(
            reason=f"sm75_rule: int8/fp8/bf16/fp16-fp32 not supported. {fmha_arg}"
        )
    _non_sm90_rule(fmha_arg)
...
def _sm90_rule(fmha_arg):
    ...
    if (
        "SM90_USE_IMMA" not in os.environ
        and fmha_arg.precision == "int8"
        and (
            fmha_arg.head_dim != 64 or fmha_arg.seq_len not in [64, 128, 256, 384, 512]
        )
    ):
        pytest.skip(
            reason=f"sm90_rule: non_64_head_size IMMA kernels not generated. {fmha_arg}"
        )
    ...
    if precision_check and (fmha_arg.head_dim not in [32, 64, 128, 256]):
        pytest.skip(
            reason=f"sm90_rule: head_dims not supported;. \
            {fmha_arg}"
        )
    ...
    if (
        ...
        and (
            fmha_arg.head_dim not in [32, 64]
            or fmha_arg.seq_len not in [64, 128, 256, 384, 512]
        )
    ):
        pytest.skip(
    ...
    if (
        (fmha_arg.force_non_warp_specialization or fmha_arg.force_non_flash_attention)
        and fmha_arg.use_tma
        and (
            fmha_arg.head_dim not in [64]
            or fmha_arg.seq_len not in [64, 128, 256]
            or fmha_arg.precision in ["fp16-fp32", "bf16"]
        )
    ):
        pytest.skip(
            reason=f"sm90_rule: cases not supported for non-flash-attention tma kernels;. \
            {fmha_arg}"
        )
    ...
    if (
        ...
        pytest.skip(
            reason=f"sm90_rule: cases not supported for non-warp-specialized flash-attention tma kernels;. \
            {fmha_arg}"
        )

    if fmha_arg.force_non_flash_attention and (
        fmha_arg.head_dim not in [16, 32, 64]
        or fmha_arg.seq_len not in [64, 128, 256, 384, 512]
    ):
        pytest.skip(
            reason=f"sm90_rule: cases not supported for non-flash-attention kernels;. \
            {fmha_arg}"
        )
