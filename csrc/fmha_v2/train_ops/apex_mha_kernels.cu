...
std::vector<torch::Tensor> fwd_cuda(torch::Tensor const& qkv, torch::Tensor const& pad_mask,
                                    float dropout_prob) {
  // sxbxhx3xd
  ...
  gemm_switch_fp32accum(a_layout_t, b_layout_n, k_seq_len, q_seq_len, head_dim, scale,
                        k_lin_results_ptr, lead_dim, batch_stride, q_lin_results_ptr, lead_dim,
                        batch_stride, beta_zero, bmm1_results_ptr, k_seq_len, k_seq_len * q_seq_len,
                        attn_batches);

  // Padded Softmax
  bool softmax_success = false;
  softmax_success = dispatch_additive_masked_softmax_dropout<half, half, float>(
      dropout_results_ptr, dropout_mask.data_ptr<uint8_t>(), bmm1_results_ptr,
      static_cast<half*>(pad_mask.data_ptr()), attn_batches * q_seq_len * q_seq_len, k_seq_len,
  ...
  gemm_switch_fp32accum(
      a_layout_n, b_layout_n, head_dim, q_seq_len, k_seq_len, alpha, v_lin_results_ptr, lead_dim,
      batch_stride, static_cast<half*>(dropout_results.data_ptr()), k_seq_len,
      k_seq_len * q_seq_len, beta_zero, static_cast<half*>(matmul2_results.data_ptr()),
...
std::vector<torch::Tensor> bwd_cuda(int heads,
  ...
  gemm_switch_fp32accum(a_layout_t, b_layout_n, k_seq_len, q_seq_len, head_dim, alpha,
                        ...
                        static_cast<half const*>(output_lin_grads.data_ptr()),
  ...
  gemm_switch_fp32accum(
      a_layout_n, b_layout_t, head_dim, k_seq_len, q_seq_len, alpha,
      static_cast<half const*>(output_lin_grads.data_ptr()), head_dim * attn_batches, head_dim,
      static_cast<half const*>(dropout_results.data_ptr()), k_seq_len, k_seq_len * q_seq_len, beta,
  ...
  dispatch_masked_scale_softmax_backward_recompute<half, half, float, false>(
      ...
      static_cast<uint8_t const*>(dropout_mask.data_ptr()), 1.0 / (1.0 - dropout_prob), k_seq_len,
      k_seq_len, attn_batches * q_seq_len / sequences, attn_batches * q_seq_len, stream);

  // Matmul1 Dgrad1          dQ = dP * k
  gemm_switch_fp32accum(
      a_layout_n, b_layout_n, head_dim, q_seq_len, k_seq_len, scale, k_lin_results_ptr, lead_dim,
  ...
  gemm_switch_fp32accum(
      a_layout_n, b_layout_t, head_dim, k_seq_len, q_seq_len, scale, q_lin_results_ptr, lead_dim,
      batch_stride, static_cast<half*>(matmul2_grads.data_ptr()), k_seq_len, k_seq_len * q_seq_len,
