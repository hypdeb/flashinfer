...
def reshape_softmax(S, b, s, h, d, warps_m, warps_n):
    ...
    assert (
        (loops == 1 and s == 128)
        or (loops == 16 and s == 256)
        or (loops == 32 and s == 512)
        or (loops == 24 and s == 384)
    ), "no.."
    quads = 8
    lohi = 2
    lr = 2
    ...
    S = (
        S.reshape(
            (b, h, loops, mmas_m, mmas_n, warps_n, warps_m, quads, 4, lohi, lr, vals)
        )
    ...
    Snp = S.cpu().numpy()
    dmask = torch.tensor(
        np.logical_not(np.signbit(Snp)), dtype=torch.float32, device=device
    )
    S = S.abs()
    return S, dmask
...
qkv_vs = qkv_vs.contiguous().view(total, h, 3, d).permute(0, 2, 1, 3).contiguous()
...
with torch.cuda.stream(stream):
    ...
    timer.start()
    for it in range(runs):
        ctx, S = mha.fwd(qkv_vs, cu_seqlens, p_dropout, s, is_training, is_nl, None)
    timer.stop()
    timer.sync()
    ...
    timer.start()
    for it in range(runs):
        dqkv_a, dU_a = apex_mha.bwd(h, Ca, Ca, Sa, Pa, mask, qkvt, Da, p_dropout)
    timer.stop()
    timer.sync()
...
print(
    "[FWD s={:d}, b={:d}] Fused {:.3f}ms Apex {:.3f}ms Diff {:.3f}ms Speedup {:.2f}x".format(
        s, b, ms_fused, ms_apex, ms_apex - ms_fused, ms_apex / ms_fused
    )
)
print(
    "[BWD s={:d}, b={:d}] Fused {:.3f}ms Apex {:.3f}ms Diff {:.3f}ms Speedup {:.2f}x".format(
        s,
        b,
        ms_fused_bwd,
        ms_apex_bwd,
