...
inline __device__ float4 ldg128(void const* ptr) { return *static_cast<float4 const*>(ptr); }

inline __device__ void stg128(void* ptr, float4 const& data) { *static_cast<float4*>(ptr) = data; }
...
__global__ __launch_bounds__(THREADS) void fmha_noloop_reduce_kernel(
    void* __restrict__ out, void const* __restrict__ in, int const* __restrict__ cu_seqlens,
