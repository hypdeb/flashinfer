...
def run_test(s, d, heads_interleaved=False, seqs_interleaved=False):
    ...
    print(
        "\n#### TEST fp16 = {}, is_causal = {}, has_alibi = {}, b = {}, h = {}, s = {}, d = {}, runs = {}, p_drop = {} ####\n".format(
            dtype == torch.float16, is_causal, has_alibi, b, h, s, d, runs, p_dropout
        )
    )
    ...
    if seqs_interleaved:
        qkv_vs = (
            qkv_vs.reshape(b, s, 3, h, d).permute(1, 0, 2, 3, 4).reshape(b * s, 3, h, d)
        )
    ...
    for _ in range(runs):
        ctx, lse, random_seed, S_mma = mha.fwd(
            qkv_vs,
            cu_seqlens,
            p_dropout,
            s,
    ...
    ctx_ref, Pref, Sref, lse_ref = mha_ref(
        qkv, amask, D, b, s, h, d, p_dropout, is_causal, alibi_bias
    )

    print("= Testing Fused Forward =")
    print(
        "[Softmax_lse] AC: {}; MAE: {:.4e}; PERR: {:.4e}".format(
    ...
    # Reduction
    tmp = (dS * Sref).sum(-1, keepdims=True)  # one reduction, similar to softmax
    ...
    if seqs_interleaved:
        dqkv2 = (
            dqkv2.reshape(s, b, 3, h, d).permute(1, 0, 3, 2, 4).reshape(b * s, h, 3, d)
        )
    else:
        dqkv2 = dqkv2.permute(0, 2, 1, 3)
