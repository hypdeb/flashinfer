...
inline __device__ void compute_dq_dk_dv_1xN_kv_inner_loop_(Params const& params, Prng& ph,
                                                           int kv_offset) {
  ...
  using Cta_tile_dk_dv =
      typename Traits::template Cta_tile_extd<Cta_tile_p::N,        // STEPK
                                              Cta_tile_p::K,        // D
                                              Cta_tile_p::M,        // STEPQ
  ...
  using Smem_tile_q_b =
      fmha::Smem_tile_b<Traits, Cta_tile_dk_dv, fmha::Row, Gmem_tile_q::BYTES_PER_LDG, 2>;
  ...
  using Smem_tile_do_a =
      fmha::Smem_tile_a<Traits, Cta_tile_p, fmha::Row, Gmem_tile_do::BYTES_PER_LDG, 2>;
  ...
  using Smem_tile_do_b =
      fmha::Smem_tile_b<Traits, Cta_tile_do_b, fmha::Row, Gmem_tile_do::BYTES_PER_LDG, 2>;
  ...
  fmha::Softmax_statistics<Traits, Cta_tile_p> sum_array(params, params.softmax_sum_ptr, binfo,
                                                         tidx);
...
#pragma unroll
    ...
    float const scale_softmax_rp_droput =
        reinterpret_cast<float const&>(params.scale_softmax) * params.rp_dropout;
...
#pragma unroll
      for (int ni = 0; ni < Mma_tile_p::MMAS_N; ++ni) {
        ...
        softmax.elt_[2 * mi + 0][4 * ni + 0] =
            pointwise_mult(softmax.elt_[2 * mi + 0][4 * ni + 0], acc_dp[mi][ni].elt(0),
                           softmax_sum[2 * mi + 0], scale_softmax_rp_droput, params.has_dropout);
        softmax.elt_[2 * mi + 0][4 * ni + 1] =
            pointwise_mult(softmax.elt_[2 * mi + 0][4 * ni + 1], acc_dp[mi][ni].elt(1),
                           softmax_sum[2 * mi + 0], scale_softmax_rp_droput, params.has_dropout);
        softmax.elt_[2 * mi + 0][4 * ni + 2] =
            pointwise_mult(softmax.elt_[2 * mi + 0][4 * ni + 2], acc_dp[mi][ni].elt(4),
                           softmax_sum[2 * mi + 0], scale_softmax_rp_droput, params.has_dropout);
        softmax.elt_[2 * mi + 0][4 * ni + 3] =
            pointwise_mult(softmax.elt_[2 * mi + 0][4 * ni + 3], acc_dp[mi][ni].elt(5),
                           softmax_sum[2 * mi + 0], scale_softmax_rp_droput, params.has_dropout);
        softmax.elt_[2 * mi + 1][4 * ni + 0] =
            pointwise_mult(softmax.elt_[2 * mi + 1][4 * ni + 0], acc_dp[mi][ni].elt(2),
                           softmax_sum[2 * mi + 1], scale_softmax_rp_droput, params.has_dropout);
        softmax.elt_[2 * mi + 1][4 * ni + 1] =
            pointwise_mult(softmax.elt_[2 * mi + 1][4 * ni + 1], acc_dp[mi][ni].elt(3),
                           softmax_sum[2 * mi + 1], scale_softmax_rp_droput, params.has_dropout);
        softmax.elt_[2 * mi + 1][4 * ni + 2] =
            pointwise_mult(softmax.elt_[2 * mi + 1][4 * ni + 2], acc_dp[mi][ni].elt(6),
                           softmax_sum[2 * mi + 1], scale_softmax_rp_droput, params.has_dropout);
        softmax.elt_[2 * mi + 1][4 * ni + 3] =
            pointwise_mult(softmax.elt_[2 * mi + 1][4 * ni + 3], acc_dp[mi][ni].elt(7),
                           softmax_sum[2 * mi + 1], scale_softmax_rp_droput, params.has_dropout);
      }
    }
...
inline __device__ void compute_dq_dk_dv_1xN_kv_inner_loop(Params const& params,
                                                          int kv_iterations = 1) {
  ...
  for (int it = 0; it < kv_iterations; it++) {
    ...
    fused_multihead_attention::compute_dq_dk_dv_1xN_kv_inner_loop_<Kernel_traits, Params>(
        params, ph, kv_offset);
  }
}
...
inline __device__ void convert_dq_to_16bits(Params const& params) {
  ...
  enum { FEATURES_PER_THREAD = 2 };

  // The number of threads per head.
  enum { THREADS_PER_TOKEN = Cta_tile_dq::N / FEATURES_PER_THREAD };

  // The valid number of threads per head.
  enum { VALID_THREADS_PER_TOKEN = Cta_tile_dq::VALID_N / FEATURES_PER_THREAD };
  ...
  // DEBUG.
  static_assert((Cta_tile_dq::N == 64 || Cta_tile_dq::N == 128) &&
                    (THREADS_PER_TOKEN == 32 || THREADS_PER_TOKEN == 64),
                "");
  // END OF DEBUG.
  ...
  for (int ii = tidx * FEATURES_PER_THREAD; ii < features_per_token; ii += features_per_stg) {
    ...
    if (lane_per_head < VALID_THREADS_PER_TOKEN) {
      ...
      uint16_t* local_ptr = output_ptr +
                            head_id_per_stg * Cta_tile_dq::VALID_N * NEXT_HEAD_OFFSET_FACTOR +
