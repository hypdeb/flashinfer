...
inline __device__ void dot_do_o(uint4 const (&do_)[M], uint4 const (&o)[M], float const scale,
                                Gmem_softmax_sum gmem_softmax_d, int tidx) {
  float sum[M];
  fmha::SumOp<float> sum_op;
#pragma unroll
  for (int mi = 0; mi < M; ++mi) {
    sum[mi] = fmha::Allreduce<THREADS_PER_ROW>::run(fmha::fma8_in_float<Data_type_>(do_[mi], o[mi]),
                                                    sum_op) *
              scale;
  }
...
inline __device__ void compute_dot_do_o(Params const& params) {
  ...
  enum { ROWS = Gmem_tile_do::ROWS };

  enum { THREADS_PER_ROW = Gmem_tile_do::THREADS_PER_ROW };

  enum { LDGS = Gmem_tile_do::LDGS };
...
inline __device__ void compute_dv_1xN(Params const& params, int const isteps) {
  ...
  using Cta_tile_dv =
      typename Traits::template Cta_tile_extd<Cta_tile_p::N, Cta_tile_p::K, Cta_tile_p::M,
                                              Cta_tile_p::K, Cta_tile_p::M, Cta_tile_p::WARPS_N, 1,
                                              Cta_tile_p::WARPS_M>;
  // Start of DEBUG
  static_assert(Cta_tile_dv::M == 512 || Cta_tile_dv::M == 384 || Cta_tile_dv::M == 256 ||
                Cta_tile_dv::M == 128 || Cta_tile_dv::M == 64);
  ...
  using Smem_tile_st =
      Smem_tile_mma_transposed<Traits,
  ...
  Smem_tile_q smem_q(&smem_[Smem_tile_do::BYTES_PER_TILE + Smem_tile_k::BYTES_PER_TILE +
                            Smem_tile_st::BYTES_PER_TILE],
                     tidx);

  Smem_tile_k smem_kt(&smem_[Smem_tile_do::BYTES_PER_TILE + Smem_tile_k::BYTES_PER_TILE +
                             Smem_tile_st::BYTES_PER_TILE + Smem_tile_q::BYTES_PER_TILE],
                      tidx);
  ...
  Softmax softmax(params,
                  &smem_[Smem_tile_do::BYTES_PER_TILE + Smem_tile_st::BYTES_PER_TILE +
                         2 * Smem_tile_k::BYTES_PER_TILE + Smem_tile_q::BYTES_PER_TILE],
                  bidb,
                  tidx);  // TODO
  ...
  for (int loop_id = 0; loop_id < isteps; loop_id++) {  // newloop : new loop
    ...
    for (int l = 0; l < STEPS; l++) {  //  loop over sequence of sO
      int const loop = l * Cta_tile_p::M;
      if (loop >= binfo.actual_seqlen) break;
...
#pragma unroll
      ...
      typename fmha::Softmax_statistics<Traits, Cta_tile_p> lse_array(params, params.lse_ptr, binfo,
                                                                      tidx);
      ...
      if (l < STEPS - 1) {  // q/dO only need to be loaded once for each outer loop
...
#pragma unroll
        for (int ni = 0; ni < N; ni++) {
          ...
          fmha::half2_to_float2(s_mat[2 * mi + 0][4 * ni + 0], s_mat[2 * mi + 0][4 * ni + 1],
                                dst.x);
          fmha::half2_to_float2(s_mat[2 * mi + 0][4 * ni + 2], s_mat[2 * mi + 0][4 * ni + 3],
                                dst.y);
          fmha::half2_to_float2(s_mat[2 * mi + 1][4 * ni + 0], s_mat[2 * mi + 1][4 * ni + 1],
                                dst.z);
          fmha::half2_to_float2(s_mat[2 * mi + 1][4 * ni + 2], s_mat[2 * mi + 1][4 * ni + 3],
                                dst.w);
        }
      }
      ...
      for (int ki = 0; ki < Mma_tile_dv::MMAS_K; ki++) {
        for (int mi = 0; mi < Mma_tile_dv::MMAS_M; mi++) {
          for (int ii = 0; ii < Smem_tile_st::Fragment::NUM_REGS; ii++) {
            frag_st[ki][mi].reg(ii) =
                fmha::hmul2(frag_st[ki][mi].reg(ii), params.scale_dropout);  // S' ^ T
...
inline __device__ void compute_dq_dk_1xN(Params const& params, int const isteps) {
  ...
  static_assert(Cta_tile_dk::M == 512 || Cta_tile_dk::M == 384 || Cta_tile_dk::M == 256 ||
                Cta_tile_dk::M == 128 || Cta_tile_dk::M == 64);
  ...
  using Gmem_tile_dq =
      Gmem_tile_dq<Traits_fp16, Cta_tile_o_fp16, Kernel_traits_fp16::HEADS_INTERLEAVED>;
  ...
  Smem_tile_st smem_s(&smem_[Smem_tile_q::BYTES_PER_TILE + Smem_tile_k::BYTES_PER_TILE +
                             Kernel_traits::Smem_tile_o::BYTES_PER_TILE],
                      tidx);
  ...
  for (int loop_id = 0; loop_id < isteps; loop_id++) {
    ...
    // 1. Load dP
    uint4 s_regs[M][N];
    gmem_s.move(0 /*q loop*/,
                newloop /*k loop*/);  // This function should be called first before loading
...
#pragma unroll
      ...
      if (l < STEPS - 1) {
        // Load next part of S
        gmem_s.move(l + 1 /*q loop*/,
                    newloop /*k loop*/);  // This function should be called first before used
        gmem_s.load(s_regs, mask);
        smem_q.move_to_next_write_buffer();
        gmem_q.move_to(l + 1);
        gmem_q.load(smem_q);
