...
struct Noloop_traits<CHUNKS, Cta_tile, 0> {
  ...
  inline __device__ Noloop_traits(int const bidc, Block_info const& binfo) : bidc_(bidc) {
...
std::tuple<int, int, int, int, int, int> work_dist(int const S, int const total_ctas,
                                                   int const heads_total) {
