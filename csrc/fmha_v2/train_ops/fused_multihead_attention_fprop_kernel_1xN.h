...
inline __device__ void device_1xN_(Params const& params, int const bidb, int const bidh,
                                   int const begin, int const steps, Prng& ph) {
  // The instruction traits.
  using Traits = typename Kernel_traits::Traits_p;
...
inline __device__ void device_1xN(Params const& params, int const num_full_heads,
                                  int const num_main_groups, int const main_group_size,
                                  int const main_steps, int const rest_steps) {
  // Cta_tile_p::N = S
  ...
  for (int it = 0; it < num_full_heads; it++) {
    ...
    fused_multihead_attention::device_1xN_<Kernel_traits, IS_TRAINING>(params, bidb, bidh, 0, STEPS,
                                                                       ph);
    __syncthreads();
  }
  ...
  if (blockIdx.x < main_group_size * num_main_groups) {
    ...
    fused_multihead_attention::device_1xN_<Kernel_traits, IS_TRAINING>(params, bidb, bidh, offset,
                                                                       main_steps, ph);
  } else {
