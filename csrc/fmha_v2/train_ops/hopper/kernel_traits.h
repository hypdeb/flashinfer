...
template <typename Traits_p_, typename Traits_o_, uint32_t S_MAX, uint32_t D, uint32_t STEP_M,
          uint32_t STEP_N, uint32_t WARPS_M, uint32_t WARPS_N, uint32_t FLAGS_>
struct Kernel_traits_fprop {
  ...
  using Cta_tile_p =
      typename Traits_p::template Cta_tile<STEP_M, STEP_N, D, WARP_GROUP_M, WARP_GROUP_N, 1>;
  // The CTA description for O = S x V.
  using Cta_tile_o =
      typename Traits_o::template Cta_tile<STEP_M, D, STEP_N, WARP_GROUP_M, 1, WARP_GROUP_N>;

  static constexpr int NUM_MATS = 3;

  // Tile to load from the stacked QKV tensor.
  using Gmem_tile_q =
      fmha::v2::Gmem_tile_qkv<Traits_p, Cta_tile_p, Traits_p::BITS_PER_ELEMENT_A, STEP_M, D, D,
                              true,   // USE_LDGSTS
                              false,  // HEADS_INTERLEAVED,
                              NUM_MATS>;

  using Gmem_tile_k =
      fmha::v2::Gmem_tile_qkv<Traits_p, Cta_tile_p, Traits_p::BITS_PER_ELEMENT_A, STEP_N, D, D,
                              true,   // USE_LDGSTS
                              false,  // HEADS_INTERLEAVED,
                              NUM_MATS>;
  ...
  enum { BUFFERS_PER_SMEM_TILE_KV = S_MAX / STEP_N };
  ...
  static constexpr fmha::Gmma_descriptor_mode GMMA_DESC_MODE_D_A =
      Cta_tile_o::K * sizeof(typename Traits_o::A_type) >= 128
          ? fmha::Gmma_descriptor_mode::SWIZZLE_128B
          : fmha::Gmma_descriptor_mode::SWIZZLE_64B;

  static constexpr fmha::Gmma_descriptor_mode GMMA_DESC_MODE_D_B =
      Cta_tile_o::K * sizeof(typename Traits_o::B_type) >= 128
          ? fmha::Gmma_descriptor_mode::SWIZZLE_128B
          : fmha::Gmma_descriptor_mode::SWIZZLE_64B;
  ...
  // B tiles.
  using Smem_tile_kt_b = fmha::Smem_tile_hopper_b<Traits_p, Cta_tile_p, fmha::Col, 16,
                                                  BUFFERS_PER_SMEM_TILE_KV, GMMA_DESC_MODE_K,
                                                  false  // USE_TMA_K
                                                  >;
  ...
  using Compute_tile_p =
      fmha::Compute_tile_with_gmma<Traits_p, Cta_tile_p, Smem_tile_q_a, Smem_tile_kt_b,
                                   Traits_p::GMMA_A_RF, Traits_p::GMMA_B_RF>;

  // o = dP x K
  using Compute_tile_o =
      fmha::Compute_tile_with_gmma<Traits_o, Cta_tile_o, Smem_tile_s_a, Smem_tile_v_b,
...
template <typename Traits_p_, typename Traits_t_, typename Traits_ds_, typename Traits_dq_,
          typename Traits_dk_, typename Traits_dv_, uint32_t S_MAX, uint32_t D, uint32_t STEP_M,
          uint32_t STEP_N, uint32_t WARPS_M, uint32_t WARPS_N, uint32_t FLAGS_>
struct Kernel_traits_dgrad {
  ...
  using Cta_tile_p =
      typename Traits_p ::template Cta_tile<STEP_M, STEP_N, D, WARP_GROUP_M, WARP_GROUP_N, 1>;
  using Cta_tile_t =
      typename Traits_t ::template Cta_tile<STEP_M, STEP_N, D, WARP_GROUP_M, WARP_GROUP_N, 1>;
  using Cta_tile_ds =
      typename Traits_ds::template Cta_tile<STEP_M, STEP_N, D, WARP_GROUP_M, WARP_GROUP_N, 1>;
  // dQ = dP  x K (like O = S x V in fprop)
  using Cta_tile_dq =
      typename Traits_dq::template Cta_tile<STEP_M, D, STEP_N, WARP_GROUP_M, 1, WARP_GROUP_N>;
  // dV = S'  x V
  using Cta_tile_dv =
      typename Traits_dv::template Cta_tile<STEP_N, D, STEP_M, WARP_GROUP_N, WARP_GROUP_M, 1>;
  // dK = dP' x Q
  using Cta_tile_dk =
      typename Traits_dk::template Cta_tile<STEP_N, D, STEP_M, WARP_GROUP_N, WARP_GROUP_M, 1>;
  ...
  using Gmem_tile_o =
      fmha::v2::Gmem_tile_qkv<Traits_t, Cta_tile_t, Traits_t::BITS_PER_ELEMENT_A, STEP_M, D, D,
                              true,   // USE_LDGSTS
                              false,  // HEADS_INTERLEAVED,
                              1       // NUM_QKV_MATS
                              >;
  ...
  // Number of buffers for Q, dO (fully buffered).
  enum { BUFFERS_PER_SMEM_TILE_Q = S_MAX / STEP_M };
  ...
  static constexpr fmha::Gmma_descriptor_mode GMMA_DESC_MODE_D_A =
      Cta_tile_dv::K * sizeof(typename Traits_dv::A_type) >= 128
          ? fmha::Gmma_descriptor_mode::SWIZZLE_128B
          : fmha::Gmma_descriptor_mode::SWIZZLE_64B;

  static constexpr fmha::Gmma_descriptor_mode GMMA_DESC_MODE_D_B =
      Cta_tile_dv::K * sizeof(typename Traits_dv::B_type) >= 128
          ? fmha::Gmma_descriptor_mode::SWIZZLE_128B
          : fmha::Gmma_descriptor_mode::SWIZZLE_64B;
  ...
  // A tiles.
  using Smem_tile_q_a = fmha::Smem_tile_hopper_a<Traits_p, Cta_tile_p, fmha::Row, 16,
                                                 BUFFERS_PER_SMEM_TILE_Q, GMMA_DESC_MODE_Q,
                                                 false,  // USE_TMA_Q,
                                                 Traits_p::GMMA_A_RF>;

  using Smem_tile_do_a = fmha::Smem_tile_hopper_a<Traits_t, Cta_tile_t, fmha::Row, 16,
                                                  BUFFERS_PER_SMEM_TILE_Q, GMMA_DESC_MODE_Q,
                                                  false,  // USE_TMA_Q,
                                                  Traits_t::GMMA_A_RF>;
  ...
  // T = dO x O'
  using Compute_tile_t =
      fmha::Compute_tile_with_gmma<Traits_t, Cta_tile_t, Smem_tile_do_a, Smem_tile_o_b,
                                   Traits_t::GMMA_A_RF, Traits_t::GMMA_B_RF>;

  // dS = dO x V'
  using Compute_tile_ds =
      fmha::Compute_tile_with_gmma<Traits_ds, Cta_tile_ds, Smem_tile_do_a, Smem_tile_vt_b,
                                   Traits_ds::GMMA_A_RF, Traits_ds::GMMA_B_RF>;

  // dQ = dP x K
  using Compute_tile_dq =
      fmha::Compute_tile_with_gmma<Traits_dq, Cta_tile_dq, Smem_tile_dp_a, Smem_tile_k_b,
                                   Traits_dq::GMMA_A_RF, Traits_dq::GMMA_B_RF>;

  // dK = dP' x Q
  using Compute_tile_dk =
      fmha::Compute_tile_with_gmma<Traits_dk, Cta_tile_dk, Smem_tile_dpt_a, Smem_tile_q_b,
                                   Traits_dk::GMMA_A_RF, Traits_dk::GMMA_B_RF>;

  // dV = S' x dO
  using Compute_tile_dv =
      fmha::Compute_tile_with_gmma<Traits_dv, Cta_tile_dv, Smem_tile_st_a, Smem_tile_do_b,
