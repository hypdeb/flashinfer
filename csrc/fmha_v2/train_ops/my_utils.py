...
def reshape_softmax(
    ...
    assert (
        ...
        or (loops == 64 and s == 1024)
        or (loops == 128 and s == 2048)
        or (loops == 256 and s == 4096)
        or (loops == 32 and s == 4096)
        or (loops == 16 and s == 1024)
        or (loops == 32 and s == 2048)
    )
    ...
    dmask = torch.tensor(
        np.logical_not(np.signbit(SS.cpu().numpy())), dtype=SS.dtype, device=SS.device
    )
...
def reshape_softmax_new(
    S: torch.Tensor, b: int, s: int, h: int, d: int, warps_m: int, warps_n: int
) -> [torch.Tensor, torch.Tensor]:
    ...
    S = (
        S.reshape(
            (b, h, loops, mmas_m, warps_m, lohi, quads, mmas_n, warps_n, lr, 4, vals)
        )
