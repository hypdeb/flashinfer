...
class _MHA(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(
            ...
            qkv_weight,
            workspace,
            fp8_meta["scaling"].scale_inv[tex.FP8Tensors.GEMM1_WEIGHT].clone().detach(),
            fp8_meta["scaling"].scale_inv[tex.FP8Tensors.GEMM1_INPUT].clone().detach(),
            qkv_out,
            M,
            Z,  # S_dmask,
            context_,
            context_t,
            proj_weight,
            fp8_meta["scaling"].scale_inv[tex.FP8Tensors.GEMM2_WEIGHT].clone().detach(),
            fp8_meta["scaling"].scale_inv[tex.FP8Tensors.GEMM2_INPUT].clone().detach(),
            # TODO remove duplicates.
            fp8_meta["scaling"].scale_inv[META_QKV].clone().detach(),  # d_scale_qkv
    ...
    def backward(
        ...
        proj_bgrad, proj_grad_output_c, proj_grad_output_t = (
            ext.fp8_cast_transpose_bgrad_fused(
                grad_output,
        ...
        ####################################################################################
        qkv_bgrad, dqkv_grad_output_c, dqkv_grad_output_t = (
            ext.fp8_cast_transpose_bgrad_fused(
                dqkv,
                ctx.fp8_meta["scaling"],
