...
at::Tensor bmm_nt(at::Tensor const& A, at::Tensor const& B, cudaDataType_t A_type,
                  cudaDataType_t B_type, float const alpha) {
  ...
  RefBMM bmm(A_type,  // CUDA_R_8F_E4M3,
             ...
             CUDA_R_32F, CUBLAS_COMPUTE_32F, CUDA_R_32F, false, true, m, n, k, strides_A[1],
             strides_B[1], n, strides_A[0], strides_B[0], m * n, b);
...
at::Tensor matmul_nt(at::Tensor const& A, at::Tensor const& B, cudaDataType_t A_type,
                     cudaDataType_t B_type, float const alpha) {
  auto dim = A.dim();
...
at::Tensor seqlens2mask(at::Tensor const& cu_seqlens, int const s,
...
std::tuple<at::Tensor, at::Tensor> full_mask(int const b, int const s,
...
std::tuple<at::Tensor, at::Tensor> rand_mask(int const b, int const s,
                                             torch::TensorOptions const& options) {
...
std::tuple<at::Tensor, at::Tensor> lin_mask(int const b, int const s,
                                            torch::TensorOptions const& options) {
  // Get a mask that represents b sequences of length randomly drawn from [1, s)
  using namespace torch::indexing;
...
std::tuple<at::Tensor, at::Tensor> make_mask(int const b, int const s,
                                             torch::TensorOptions const& options,
                                             Maskgen const maskgen) {
  at::Tensor cu_seqlens, amask;
  switch (maskgen) {
    case RANDOM:
      std::tie(cu_seqlens, amask) = rand_mask(b, s, options);
      break;
    case LINEAR:
      std::tie(cu_seqlens, amask) = lin_mask(b, s, options);
      break;
    default:
      std::tie(cu_seqlens, amask) = full_mask(b, s, options);
      break;
  }
  return {cu_seqlens, amask};
}

at::Tensor draw_tensor(at::IntArrayRef const dims, torch::TensorOptions const& options,
                       Datagen const datagen) {
  switch (datagen) {
    case SMALLINT:
      return torch::randint(-2, 3, dims, options);
    case ONES:
...
int check_results(at::Tensor const& out, at::Tensor const& ref, float epsilon, bool verbose,
                  bool with_colors) {
  int m = out.size(-1);
  ...
  return check_results(out_h.data_ptr<float>(), ref_h.data_ptr<float>(), m, n, out_h.stride(0),
                       epsilon, verbose, with_colors);
}
