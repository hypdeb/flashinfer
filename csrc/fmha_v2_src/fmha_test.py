...
@pytest.mark.parametrize(
    "dtype", ["-bf16", "-e4m3", "-e4m3 -bf16-output"], ids=["bf16", "e4m3", "e4m3-bf16"]
)
@pytest.mark.parametrize("s", [1024, 4096], ids=["seqlen-1024", "seqlen-4096"])
...
@pytest.mark.parametrize(
    ...
    [16, 32, 64, 128],
    ids=[
        "num-grouped-heads-16",
        "num-grouped-heads-32",
        "num-grouped-heads-64",
        "num-grouped-heads-128",
    ],
)
...
@pytest.mark.parametrize(
    "mask", ["-causal-mask", ""], ids=["causal-mask", "padding-mask"]
)
@pytest.mark.parametrize(
    "s",
    [128, 256, 384, 512],
...
@pytest.mark.parametrize(
    "input_layout", ["", "-paged-kv"], ids=["packed-qkv", "paged-kv"]
)
def test_trtllm_chunked_attention(chunked_attention_size, input_layout):
    ...
    subprocess.run(
        f"bin/fmha.exe -d 128 -b 4 -h 5 -fp16 -s 8192 -min-s 4096 \
