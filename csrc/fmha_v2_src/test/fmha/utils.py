...
FmhaArgs = namedtuple(
    "FmhaArgs",
    fields_fmha,
    defaults=(256, 1, 64, 1, 1, "fp16", False, False, True, False),
)
FmhcaArgs = namedtuple(
    "FmhcaArgs", fields_fmhca, defaults=(256, 256, 1, 64, 1, 1, "fp16", False)
)
...
def combinations_base():
    ...
    fmha_args_increment1 = fmha_args + [
        fmha_arg._replace(min_seq_len=fmha_arg.seq_len) for fmha_arg in fmha_args
    ]
...
def reduced_combinations_fp16():
    ...
    return [fmha_arg for fmha_arg in fmha_args if fmha_arg.seq_len in [128, 256, 512]]
...
def reduced2x_combinations_fp16():
    ...
    return [
        ...
        if fmha_arg.seq_len in [128, 256, 512] and fmha_arg.head_dim in [32, 64]
    ]
...
def combinations_fmhca():
    ...
    fmha_args_increment1 = [
        fmha_arg._replace(min_seq_len=fmha_arg.seq_len_q) for fmha_arg in fmha_args
    ]
    ...
    **kwargs: optional kwargs to pass to rules
    ...
    for k, v in fmha_arg._asdict().items():
        if k == "precision":
            ...
            k == "use_interleaved"
            or k == "use_tma"
            or k == "force_non_warp_specialization"
