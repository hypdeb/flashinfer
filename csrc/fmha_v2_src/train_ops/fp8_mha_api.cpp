...
static inline bool check_tensor(at::Tensor const& x) { return x.is_cuda() && x.is_contiguous(); }
...
std::vector<at::Tensor> mha_fwd(
    ...
    at::Tensor const& d_scale_qkv, at::Tensor const& q_scale_s, at::Tensor const& q_scale_o,
    at::Tensor& amax_s, at::Tensor& amax_o,
  ...
  typename fprop::Launch_params::Params params_(
      QKV8.data_ptr(), O8.data_ptr(), Mfprop.data_ptr<float>(), Zfprop.data_ptr<float>(),
      cu_seqlens.data_ptr<int>(), amax_s.data_ptr<float>(), amax_o.data_ptr<float>(), b, s, h, d,
      p_dropout, scale_q_k, d_scale_qkv.data_ptr<float>(), d_scale_s.data_ptr<float>(),
...
std::vector<at::Tensor> mha_bwd(
    at::Tensor const& dO8,         // total x num_heads, x head_size
    at::Tensor const& QKV8,        // total x num_heads x 3 x head_size, total := \sum_{i=0}^{b} s_i
  ...
  typename dgrad::Launch_params::Params params_(
      dQKV8.data_ptr(), QKV8.data_ptr(), dO8.data_ptr(), O8.data_ptr(), Mfprop.data_ptr<float>(),
      Zfprop.data_ptr<float>(), cu_seqlens.data_ptr<int>(), amax_dp.data_ptr<float>(),
      amax_dqkv.data_ptr<float>(), dQtmp.data_ptr(), b, s, h, d, p_dropout, scale_q_k,
      d_scale_qkv.data_ptr<float>(), d_scale_s.data_ptr<float>(), d_scale_o.data_ptr<float>(),
      d_scale_do.data_ptr<float>(), d_scale_dp.data_ptr<float>(), d_scale_dqkv.data_ptr<float>(),
      q_scale_s.data_ptr<float>(), q_scale_dp.data_ptr<float>(), q_scale_dqkv.data_ptr<float>(),
      reinterpret_cast<uint64_t*>(philox_unpacked.data_ptr()));
