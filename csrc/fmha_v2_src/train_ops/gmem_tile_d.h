...
struct Gmem_tile_mma_sd {
  ...
  enum { STEPK = Cta_tile::N };

  enum { STEPQ = Cta_tile::M };

  using Type = typename fmha::Uint_from_size_in_bytes<BYTES_PER_STG>::Type;

  template <typename Params, typename Block_info>
  inline __device__ Gmem_tile_mma_sd(void* ptr, Params const& params, Block_info const& block_info,
                                     int tidx)
  ...
  inline __device__ void move_k() { ptr_ += MMAS_N * BYTES_PER_ROW; }
  ...
  inline __device__ void move(int qstep_id, int kstep_id) {
    ptr_ = init_ptr_ + qstep_id * actual_seqlen_ * STEPQ * BYTES_PER_ELEMENT +
           kstep_id * MMAS_N * BYTES_PER_ROW;
  }
...
template <typename Traits, typename Cta_tile,
          typename Base = Gmem_tile_mma_sd<Traits, Cta_tile, sizeof(uint16_t)>>
...
#pragma unroll
  ...
  template <typename Mask>
  inline __device__ void softmax_to_regs(float const (&softmax)[2 * M][4 * N], uint4 (&regs)[M][N],
                                         Mask const& mask) {
#pragma unroll
    for (int mi = 0; mi < M; mi++) {
#pragma unroll
...
template <
    ...
    // The base class.
    typename Base =
        fmha::v2::Gmem_tile_qkv<Traits, Cta_tile, Traits::BITS_PER_ELEMENT_A, Cta_tile::M,
                                Cta_tile::K, Cta_tile::VALID_K, false, false>>
struct Gmem_tile_dout : public Base {
  ...
  static_assert(Base::BYTES_PER_ROW == 128 || Base::BYTES_PER_ROW == 256 ||
                Base::BYTES_PER_ROW == 512);
  ...
  inline __device__ Gmem_tile_dout(Params const& params, Block_info const& block_info, int tidx)
    ...
    int64_t row_offset = (int64_t)this->row_ * params.o_stride_in_bytes +
                         block_info.bidx * Base::VALID_BYTES_PER_ROW;
...
struct Gmem_tile_out : public Base {
  ...
  enum { ROWS_PER_LDG = Base::ROWS_PER_LDG };
...
struct Gmem_softmax_sum {
  // The helper class.
  using Helper = fmha::v2::Gmem_tile_qkv<Traits, Cta_tile, Traits::BITS_PER_ELEMENT_A, Cta_tile::M,
                                         Cta_tile::K, Cta_tile::VALID_K, false, false>;
  // The Mma tile.
  using Mma_tile = typename Traits::template Mma_tile<Cta_tile>;
  ...
  enum { BYTES_PER_MMA = (Cta_tile::THREADS_PER_WARP / 4) * 2 * BYTES_PER_ELEMENT };

  // The number of rows produced by iteration (STEPQ).
  enum { ROWS = Cta_tile::M };

  // The number of rows written per LDG.
  enum { ROWS_PER_LDG = Helper::ROWS_PER_LDG };

  // The number of LDGs.
  enum { LDGS = Helper::LDGS };

  // Ctor.
  template <typename Params>
  inline __device__ Gmem_softmax_sum(Params const& params, int const tidx)
      : ptr_(reinterpret_cast<char*>(params.softmax_sum_ptr)), tidx_(tidx) {
...
#pragma unroll
  ...
  inline __device__ void store_row(uint32_t const (&data)[LDGS], int const row) {
    enum { BYTES_PER_LDG = ROWS_PER_LDG * BYTES_PER_ELEMENT };
...
struct Gmem_tile_dq_red_base {
  ...
  enum { THREADS_PER_ROW = ELEMENTS_PER_ROW };
