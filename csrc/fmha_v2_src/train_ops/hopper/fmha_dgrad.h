...
struct Fmha_dgrad_params {
  Fmha_dgrad_params(void* dqkv_ptr_, void* qkv_ptr_, void* do_ptr_, void* o_ptr_, float* m_ptr_,
                    float* zi_ptr_, int* cu_seqlens_, float* amax_dp_, float* amax_dqkv_,
                    void* dq_tmp_ptr_, int b_, int s_, int h_, int d_, float p_dropout,
                    float scale_bmm_q_k_, float* ptr_d_scale_qkv_, float* ptr_d_scale_s_,
                    float* ptr_d_scale_o_, float* ptr_d_scale_do_, float* ptr_d_scale_dp_,
                    float* ptr_d_scale_dqkv_, float* ptr_q_scale_s_, float* ptr_q_scale_dp_,
                    float* ptr_q_scale_dqkv_, uint64_t* ptr_philox_unpacked_)
      : dqkv_ptr(dqkv_ptr_),
        qkv_ptr(qkv_ptr_),
        do_ptr(do_ptr_),
        o_ptr(o_ptr_),
...
struct Launch_params_ {
  ...
  Launch_params_(cudaDeviceProp* props_, cudaStream_t stream_, Params params_,
