...
#pragma unroll
  ...
  __device__ inline Dgrad_4x1(Fmha_dgrad_params const& params, int const bidb, int const bidh,
                              int const tidx,
  ...
  __device__ inline void operator()(uint64_t seed, uint64_t offset) {
    ...
    Gmem_tile_dq gmem_dq8(params_.dqkv_ptr, params_.qkv_stride_in_bytes, binfo_, tidx_,
                          q_scale_dqkv, 0, 0);
    Gmem_tile_dk gmem_dk8(params_.dqkv_ptr, params_.qkv_stride_in_bytes, binfo_, tidx_,
                          q_scale_dqkv, 0, 1);
    Gmem_tile_dv gmem_dv8(params_.dqkv_ptr, params_.qkv_stride_in_bytes, binfo_, tidx_,
                          q_scale_dqkv, 0, 2);
    ...
    for (int row = 0; row < steps - 1; ++row) {
      ...
      // T  = dO x O'
      Compute_tile_t ctile_t(smem_nvvm_do_a,
                             smem_nvvm_ot_b + (row % 2) * Smem_tile_o_b::BYTES_PER_BUFFER);
      ctile_t.clear();
    ...
    for (int ci = 0; ci < steps; ++ci) {
      ...
      for (int ri = 0; ri < steps; ++ri) {
        ...
        if (!(ci == 0 && ri == 0)) __syncthreads();
        if (first_col && last_col) {
          compute_single_tile<true, true>(ctile_dk, ctile_dv, ctile_p, ctile_ds, ctile_dq,
                                          smem_nvvm_do_a, smem_nvvm_do_b, smem_nvvm_q_a,
                                          smem_nvvm_q_b, tidx_, ri, ci, gmem_dq8, ph);
        } else if (first_col && !last_col) {
          compute_single_tile<true, false>(ctile_dk, ctile_dv, ctile_p, ctile_ds, ctile_dq,
                                           smem_nvvm_do_a, smem_nvvm_do_b, smem_nvvm_q_a,
                                           smem_nvvm_q_b, tidx_, ri, ci, gmem_dq8, ph);
        } else if (!first_col && last_col) {
          compute_single_tile<false, true>(ctile_dk, ctile_dv, ctile_p, ctile_ds, ctile_dq,
                                           smem_nvvm_do_a, smem_nvvm_do_b, smem_nvvm_q_a,
                                           smem_nvvm_q_b, tidx_, ri, ci, gmem_dq8, ph);
        } else {  //! first_col && !last_col
          compute_single_tile<false, false>(ctile_dk, ctile_dv, ctile_p, ctile_ds, ctile_dq,
                                            smem_nvvm_do_a, smem_nvvm_do_b, smem_nvvm_q_a,
                                            smem_nvvm_q_b, tidx_, ri, ci, gmem_dq8, ph);
        }
...
#pragma unroll
    ...
    // Also the compiler generates REDUX for intra-warp reduction and calls atomicMax only once per
    // warp.
  ...
  __device__ inline void compute_single_tile(
      Compute_tile_dk& ctile_dk, Compute_tile_dv& ctile_dv, Compute_tile_p& ctile_p,
      Compute_tile_ds& ctile_ds, Compute_tile_dq& ctile_dq, uint32_t const smem_nvvm_do_a,
      uint32_t const smem_nvvm_do_b, uint32_t const smem_nvvm_q_a, uint32_t const smem_nvvm_q_b,
      int const tidx, int const ri, int const ci, Gmem_tile_dq& gmem_dq8, Philox& ph) {
    if (IS_FIRST_COL) {
      ctile_dq.clear();
    } else {
...
#pragma unroll
    ...
    // uint32_t smem_nvvm_kt_b = __nvvm_get_smem_pointer(&smem_->kt_b) + (ci % 2) *
    // Smem_tile_o_b::BYTES_PER_BUFFER; uint32_t smem_nvvm_k_b  =
    // __nvvm_get_smem_pointer(&smem_->k_b[0]); Transposer::template transpose_<false>(tidx_,
    // smem_nvvm_kt_b, smem_nvvm_k_b);
    //  Broadcast diag_doo.
...
#pragma unroll
    for (int ni = 0; ni < Mma_tile_p::CORES_N / 2 / 2; ni++) {
      frag_dp[ni].reg(0) =
          fmha::half4_to_fp8x4<A_type_dq>(dp_half[0][4 * ni + 0], dp_half[0][4 * ni + 1]);
      frag_dp[ni].reg(1) =
          fmha::half4_to_fp8x4<A_type_dq>(dp_half[1][4 * ni + 0], dp_half[1][4 * ni + 1]);
      frag_dp[ni].reg(2) =
          fmha::half4_to_fp8x4<A_type_dq>(dp_half[0][4 * ni + 2], dp_half[0][4 * ni + 3]);
      frag_dp[ni].reg(3) =
          fmha::half4_to_fp8x4<A_type_dq>(dp_half[1][4 * ni + 2], dp_half[1][4 * ni + 3]);
    }
...
#pragma unroll
    ...
    size_t sbhs_tile_offset_bytes =
        ri * Cta_tile_p::M * params_.ds_stride_in_bytes + ci * Cta_tile_p::N * 4;
    Gmem_tile_p gmem_s(static_cast<char*>(params_.s_ptr) + sbhs_tile_offset_bytes,
                       params_.ds_stride_in_bytes, params_.s * 4, scale_, tidx_);

    Gmem_tile_ds gmem_ds(static_cast<char*>(params_.ds_ptr) + sbhs_tile_offset_bytes,
                         params_.ds_stride_in_bytes, params_.s * 4, scale_, tidx_);

    Gmem_tile_p gmem_dp(static_cast<char*>(params_.dp_ptr) + sbhs_tile_offset_bytes,
                        params_.ds_stride_in_bytes, params_.s * 4, scale_, tidx_);
  ...
  __device__ inline void compute_single_diag_doo(float* diag_doo, Compute_tile_t& ctile_t,
                                                 int const row_local, int const qid) {
    fmha::warpgroup_arrive();
#pragma unroll
    for (int ki = 0; ki < Mma_tile_p::MMAS_K - 1; ++ki) {
      ctile_t.compute(ki);
