...
struct Fmha_fprop_params {
  Fmha_fprop_params(void* qkv_ptr_, void* o_ptr_, float* m_ptr_, float* zi_ptr_, int* cu_seqlens_,
                    float* amax_s_, float* amax_o_, int b_, int s_, int h_, int d_, float p_dropout,
                    float scale_bmm_q_k_, float* ptr_d_scale_qkv_, float* ptr_d_scale_s_,
                    float* ptr_d_scale_o_, float* ptr_q_scale_s_, float* ptr_q_scale_o_,
...
struct Launch_params_ {
  ...
  Launch_params_(cudaDeviceProp* props_, cudaStream_t stream_, Params params_, bool is_training_)
      : props(props_), stream(stream_), params(params_), is_training(is_training_) {}
