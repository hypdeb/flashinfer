...
struct Fprop_4x1 {
  ...
  __device__ inline Fprop_4x1(Fmha_fprop_params const& params, int const bidb, int const bidh,
                              int const tidx, char* smem)
      : params_(params),
  ...
  __device__ inline void operator()(uint64_t seed, uint64_t offset) {
    ...
    for (int ri = 0; ri < steps_m; ++ri) {
      ...
      for (int ci = 1; ci < steps_n; ++ci) {
        Philox ph(seed, binfo_.tidx_global + (ri * steps_n + ci) * ELTS_PER_TILE_PER_THREAD,
                  offset);
...
#pragma unroll
      for (int mi = 0; mi < Mma_tile_p::CORES_M; mi++) {
        ...
        // No need for abs :)
        // We use the fact that for stable softmax, the largest value occurs at the position of the
        // max. That position will have value 1/p_sum.
...
#pragma unroll
  ...
  __device__ inline void compute_single_tile(Compute_tile_p& ctile_p, Compute_tile_o& ctile_o,
                                             float (&p_max)[2], float (&p_sum)[2], int const tidx,
                                             int const ri, int const ci, Philox& ph) {
    ctile_p.clear();
...
#pragma unroll
    for (int ni = 0; ni < Mma_tile_p::CORES_N * 2 / 8; ni++) {
      frag_p[ni].reg(0) = fmha::float4_to_fp8x4<A_type_o>(p_[0][8 * ni + 0], p_[0][8 * ni + 1],
                                                          p_[0][8 * ni + 2], p_[0][8 * ni + 3]);
      frag_p[ni].reg(1) = fmha::float4_to_fp8x4<A_type_o>(p_[1][8 * ni + 0], p_[1][8 * ni + 1],
                                                          p_[1][8 * ni + 2], p_[1][8 * ni + 3]);
      frag_p[ni].reg(2) = fmha::float4_to_fp8x4<A_type_o>(p_[0][8 * ni + 4], p_[0][8 * ni + 5],
                                                          p_[0][8 * ni + 6], p_[0][8 * ni + 7]);
      frag_p[ni].reg(3) = fmha::float4_to_fp8x4<A_type_o>(p_[1][8 * ni + 4], p_[1][8 * ni + 5],
                                                          p_[1][8 * ni + 6], p_[1][8 * ni + 7]);
    }
...
#pragma unroll
    ...
    Gmem_tile_p gmem_p(static_cast<char*>(params_.p_ptr) + sbhs_tile_offset_bytes,
                       params_.s_stride_in_bytes, params_.s * 4, scale_, tidx_);
