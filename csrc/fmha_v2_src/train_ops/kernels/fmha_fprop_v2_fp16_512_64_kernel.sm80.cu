...
using Kernel_traits =
    fmha::Kernel_traits_v2<fmha::Ampere_hmma_fp32_traits, 512, 64, 16, 1, 8, 1, 0x20u>;
static_assert(Kernel_traits::VERSION == 2);
static_assert(!Kernel_traits::HEADS_INTERLEAVED);

template <bool IS_TRAINING>
__global__ void fmha_fprop_v2_fp16_512_64_sm80_kernel(Fused_multihead_attention_fprop_params params,
                                                      int const total_heads) {
  fused_multihead_attention::device_1xN<Kernel_traits, IS_TRAINING>(params, total_heads);
}

template <bool IS_TRAINING>
__global__ void fmha_fprop_v2_fp16_512_64_sm80_kernel_nl(
    Fused_multihead_attention_fprop_params params, int const num_full_heads,
    int const num_main_groups, int const main_group_size, int const main_steps,
...
void run_fmha_v2_fp16_512_64_sm80_(
    Launch_params<Fused_multihead_attention_fprop_params>& launch_params, bool const configure) {
  auto kernel = launch_params.is_training ? &fmha_fprop_v2_fp16_512_64_sm80_kernel<true>
                                          : &fmha_fprop_v2_fp16_512_64_sm80_kernel<false>;
  ...
  int ctas_per_sm;
  FMHA_CHECK_CUDA(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&ctas_per_sm, kernel,
                                                                Kernel_traits::THREADS, smem_size));
...
void run_fmha_v2_fp16_512_64_sm80_nl_(
    Launch_params<Fused_multihead_attention_fprop_params>& launch_params, bool const configure) {
  auto kernel = launch_params.is_training ? &fmha_fprop_v2_fp16_512_64_sm80_kernel_nl<true>
                                          : &fmha_fprop_v2_fp16_512_64_sm80_kernel_nl<false>;
  ...
  if (configure) {
    ...
    std::tie(launch_params.num_full_heads, launch_params.num_main_groups,
             launch_params.heads_last_wave, launch_params.main_steps, launch_params.rest_steps,
  ...
  kernel<<<grid, Kernel_traits::THREADS, smem_size, launch_params.stream>>>(
      launch_params.params, launch_params.num_full_heads, launch_params.num_main_groups,
      launch_params.heads_last_wave, launch_params.main_steps, launch_params.rest_steps);

  FMHA_CHECK_CUDA(cudaPeekAtLastError());
}

void run_fmha_v2_fp16_512_64_sm80(
    Launch_params<Fused_multihead_attention_fprop_params>& launch_params, bool const configure) {
  if (launch_params.is_nl) {
    run_fmha_v2_fp16_512_64_sm80_nl_(launch_params, configure);
  } else {
    run_fmha_v2_fp16_512_64_sm80_(launch_params, configure);
  }
}
