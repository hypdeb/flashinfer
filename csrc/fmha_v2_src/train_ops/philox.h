...
class Philox {
 public:
  __device__ inline Philox(unsigned long long seed, unsigned long long subsequence,
  ...
  __device__ inline uint4 operator()(unsigned long long const subsequence) {
    ...
    //     printf("tidx = %d, counter_: %u, %u, %u, %u\n", threadIdx.x, counter_.x, counter_.y,
    //     counter_.z, counter_.w);
    // }
    uint2 key_ = key;
// 7-round philox
#pragma unroll
...
struct PCGenerator {
  ...
  DI uint4 operator()() { return {next_u32(), next_u32(), next_u32(), next_u32()}; }
