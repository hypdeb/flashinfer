...
std::array<at::Tensor, 3> fprop_seq(at::Tensor const& Q8, at::Tensor const& K8,
                                    at::Tensor const& V8t, at::Tensor const& amask,
                                    float const scale_q_k, float const d_scale_qkv,
                                    at::Tensor const& amax_exp_p, MetaMap const& recipe) {
  ...
  auto Pc =
      matmul_nt(Q8, K8c, recipe.at("QKV").dtype, recipe.at("QKV").dtype, d_scale_q_k * scale_q_k);
  ...
  for (int step = 1; step < steps; step++) {
    ...
    // BMM2.
    auto Oc =
        matmul_nt(ExpP8, V8c, recipe.at("expP").dtype, recipe.at("QKV").dtype, d_scale_exp_p_v);
    O = O * correction + Oc;
  }
...
std::array<at::Tensor, 18> reference(at::Tensor QKV8, at::Tensor dO8_, at::Tensor& amask,
                                     float const scale_q_k, float const d_scale_qkv,
                                     float const d_scale_do, MetaMap const& recipe) {
  // [b,s,h,d] => [b,h,s,d]
  ...
  float d_scale_q_k = d_scale_qkv * d_scale_qkv;
  auto P =
      matmul_nt(Q8, K8, recipe.at("QKV").dtype, recipe.at("QKV").dtype, d_scale_q_k * scale_q_k);
  ...
  auto amax_s = S.max();
  float q_scale_exp_p =
      recipe.at("expP").q_scale(amax_exp_p.item<float>());  // MAX_E4M3 / amax_exp_p.item<float>();
  float q_scale_s =
      recipe.at("S").q_scale(amax_s.item<float>());  // MAX_E4M3 / amax_s.item<float>();
  auto ExpP8 = convert_fp32_to_fp8(ExpP, q_scale_exp_p, recipe.at("expP").dtype);
  ...
  auto [Oseq, Zinvseq, Mseq] =
      fprop_seq(Q8, K8, V8t, amask, scale_q_k, d_scale_qkv, amax_exp_p, recipe);
  ...
  float q_scale_o =
      recipe.at("O").q_scale(amax_o.item<float>());  // MAX_E4M3 / amax_o.item<float>();
  ...
  float q_scale_dp =
      recipe.at("dP").q_scale(amax_dp.item<float>());  // MAX_E5M2 / amax_dp.item<float>();
  ...
  // dK = dP' x Q
  auto dK = matmul_nt(dP8.permute({0, 1, 3, 2}).contiguous(), Q8t, recipe.at("dP").dtype,
                      recipe.at("QKV").dtype, d_scale_dp_qkv);
  // dV = S' x dO
  auto dV = matmul_nt(S8.permute({0, 1, 3, 2}).contiguous(), dO8t, recipe.at("S").dtype,
                      recipe.at("dO").dtype, d_scale_s_do);

  auto amax_dqkv =
      torch::concat({dQ.abs().max().view(1), dK.abs().max().view(1), dV.abs().max().view(1)}).max();
  float q_scale_dqkv =
      recipe.at("dQKV").q_scale(amax_dqkv.item<float>());  // MAX_E4M3 / amax_dqkv.item<float>();
  ...
  return {P,  S,   M,   Zinv, O8,     dS,         dP,     dQ,      dK,
          dV, dQ8, dK8, dV8,  amax_s, amax_exp_p, amax_o, amax_dp, amax_dqkv};
}
...
int main(int argc, char** argv) {
  ...
  auto [Pref, Sref, M, Zinv, O8ref, dSref, dPref, dQref, dKref, dVref, dQ8ref, dK8ref, dV8ref,
        amax_s_ref, amax_exp_p_ref, amax_o_ref, amax_dp_ref, amax_dqkv_ref] =
      reference(QKV8, dO8, amask, FLAGS_scale_q_k, d_scale_qkv, d_scale_do, recipe);
  FMHA_CHECK_CUDA(cudaDeviceSynchronize());
  float q_scale_s =
      recipe.at("S").q_scale(amax_s_ref.item<float>());  // MAX_E4M3 / amax_s_ref.item<float>();
  float q_scale_dp =
      recipe.at("dP").q_scale(amax_dp_ref.item<float>());  // MAX_E5M2 / amax_dp_ref.item<float>();
  float q_scale_o =
      recipe.at("O").q_scale(amax_o_ref.item<float>());  // MAX_E4M3 / amax_o_ref.item<float>();
  float q_scale_dqkv = recipe.at("dQKV").q_scale(
      amax_dqkv_ref.item<float>());  // MAX_E4M3 / amax_dqkv_ref.item<float>();
  float q_scale_exp_p = recipe.at("expP").q_scale(
      amax_exp_p_ref.item<float>());  // MAX_E4M3 / amax_exp_p_ref.item<float>();
  ...
  FP8DgradMeta dgrad_meta(d_scale_qkv, d_scale_s_ref, d_scale_o_ref, d_scale_do, d_scale_dp_ref,
                          q_scale_s, q_scale_dp, q_scale_dqkv, options);

  typename dgrad::Launch_params::Params params_(
      dQKV8unpad.data_ptr(), QKV8unpad.data_ptr(), dO8unpad.data_ptr(), O8unpad_ref.data_ptr(),
      M.data_ptr<float>(), Zinv.data_ptr<float>(), cu_seqlens.data_ptr<int>(),
      amax_dp.data_ptr<float>(), amax_dqkv.data_ptr<float>(), dQtmp.data_ptr(), b, s, h, d,
      FLAGS_p_dropout, FLAGS_scale_q_k, dgrad_meta.d_scale_qkv.data_ptr<float>(),
      dgrad_meta.d_scale_s.data_ptr<float>(), dgrad_meta.d_scale_o.data_ptr<float>(),
      dgrad_meta.d_scale_do.data_ptr<float>(), d_scale_dp.data_ptr<float>(),
      d_scale_dqkv.data_ptr<float>(), dgrad_meta.q_scale_s.data_ptr<float>(),
      dgrad_meta.q_scale_dp.data_ptr<float>(), dgrad_meta.q_scale_dqkv.data_ptr<float>(),
  ...
  // cudaProfilerStart();
  for (int it = 0; it < FLAGS_num_iters; it++) launch(launch_params, /*configure=*/false);
  ...
  auto dQ32ref =
      convert_fp8_to_fp32(dQ8ref.contiguous(), d_scale_dqkv_ref, recipe.at("dQKV").dtype);
  auto dK32ref =
      convert_fp8_to_fp32(dK8ref.contiguous(), d_scale_dqkv_ref, recipe.at("dQKV").dtype);
  auto dV32ref =
      convert_fp8_to_fp32(dV8ref.contiguous(), d_scale_dqkv_ref, recipe.at("dQKV").dtype);

  // [b,s,h,d] => [b,h,s,d]
  auto dQ8 =
      dQKV8.index({Slice(), Slice(), 0, Slice(), Slice()}).permute({0, 2, 1, 3}).contiguous();
  auto dK8 =
      dQKV8.index({Slice(), Slice(), 1, Slice(), Slice()}).permute({0, 2, 1, 3}).contiguous();
  auto dV8 =
      dQKV8.index({Slice(), Slice(), 2, Slice(), Slice()}).permute({0, 2, 1, 3}).contiguous();
  ...
  auto tmp =
      torch::zeros({4}, options);  // convert needs multiple of 4 for vectorized loads/stores.

  printf("[amax_dp  ] CMP: %f REF(self): %f REF(ref): %f\n", amax_dp.item<float>(),
         dP.abs().max().item<float>(), dPref.abs().max().item<float>());

  tmp[0] = amax_dqkv[0];
  printf("[amax_dqkv] CMP: %f CMP(FP8): %f REF(FP8): %f\n", amax_dqkv.item<float>(),
         convert_fp8_to_fp32(convert_fp32_to_fp8(tmp, q_scale_dqkv, recipe.at("dQKV").dtype),
                             d_scale_dqkv_ref, recipe.at("dQKV").dtype)[0]
             .item<float>(),
  ...
  FP8FpropMeta fprop_meta(d_scale_qkv, d_scale_s_ref, q_scale_s, q_scale_o, options);
  {
    ...
    typename fprop::Launch_params::Params params_(
        QKV8unpad.data_ptr(), O8unpad.data_ptr(), Mfprop.data_ptr<float>(),
        Zfprop.data_ptr<float>(), cu_seqlens.data_ptr<int>(), amax_s.data_ptr<float>(),
        amax_o.data_ptr<float>(), b, s, h, d, FLAGS_p_dropout, FLAGS_scale_q_k,
        fprop_meta.d_scale_qkv.data_ptr<float>(), d_scale_s.data_ptr<float>(),
        d_scale_o.data_ptr<float>(), fprop_meta.q_scale_s.data_ptr<float>(),
        fprop_meta.q_scale_o.data_ptr<float>(),
        reinterpret_cast<uint64_t*>(philox_unpacked.data_ptr()));
    ...
    if (maskgen == Maskgen::FULL) {
      ...
      cout << dmask_d.abs().sum().item<float>() / dmask_d.numel()
           << " p_keep: " << 1.f - FLAGS_p_dropout << endl;
      cout << dmask_f.abs().sum().item<float>() / dmask_f.numel()
           << " p_keep: " << 1.f - FLAGS_p_dropout << endl;
    }

    printf("[amax_s] CMP: %f REF(self): %f REF(ref): %f\n", amax_s.item<float>(),
           Sfprop.abs().max().item<float>(), Sref.abs().max().item<float>());

    tmp[0] = amax_o[0];
    printf("[amax_o] CMP: %f CMP(FP8): %f REF(FP8): %f\n", amax_o.item<float>(),
           convert_fp8_to_fp32(convert_fp32_to_fp8(tmp, q_scale_o, recipe.at("O").dtype),
                               d_scale_o_ref, recipe.at("O").dtype)[0]
               .item<float>(),
  ...
  if (verbose > 1) {
    auto print = torch::from_blob(print_buf.data_ptr(), {8}, {1}, torch::Deleter(),
