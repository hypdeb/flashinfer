...
at::Tensor matmul_nt(at::Tensor const& A, at::Tensor const&, cudaDataType_t A_type,
                     cudaDataType_t B_typeB, float const alpha);
...
struct FP8FpropMeta {
  FP8FpropMeta(float const d_scale_qkv_, float const d_scale_s_, float const q_scale_s_,
               float const q_scale_o_, at::TensorOptions& options)
...
struct FP8DgradMeta {
  FP8DgradMeta(float const d_scale_qkv_, float const d_scale_s_, float const d_scale_o_,
               float const d_scale_do_, float const d_scale_dp_, float const q_scale_s_,
               float const q_scale_dp_, float const q_scale_dqkv_, at::TensorOptions& options)
      : d_scale_qkv(torch::full({1}, d_scale_qkv_, options.dtype(torch::kFloat32))),
        d_scale_s(torch::full({1}, d_scale_s_, options.dtype(torch::kFloat32))),
