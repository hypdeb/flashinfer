...
def generate_ninja_build_for_op(spec: JitSpec) -> str:
    system_includes = [
    ...
    if spec.extra_include_dirs is not None:
        for extra_dir in spec.extra_include_dirs:
    ...
    if spec.extra_cflags is not None:
        cflags += spec.extra_cflags
    ...
    cuda_cflags += _get_cuda_arch_flags(spec.extra_cuda_cflags)
    if spec.extra_cuda_cflags is not None:
        cuda_cflags += spec.extra_cuda_cflags
    ...
    if spec.extra_ldflags is not None:
        ldflags += spec.extra_ldflags
    ...
    lines.append("")
    link_rule = "nvcc_link" if spec.needs_device_linking else "link"
