#!/usr/bin/env python3
"""Plot compute and memory throughput from CUTLASS benchmark results."""

import pandas as pd
import matplotlib.pyplot as plt

# Read the CSV file
df = pd.read_csv("ml3_moe_cutlass_out.csv")

# Filter data by cutlass_variant
df_fp8 = df[df["cutlass_variant"] == "fp8"].copy()
df_nvfp4 = df[df["cutlass_variant"] == "nvfp4"].copy()

# Extract relevant columns (num_tokens appears multiple times, use iloc[:, 29])
num_tokens_fp8 = df_fp8.iloc[:, 29].astype(int)
num_tokens_nvfp4 = df_nvfp4.iloc[:, 29].astype(int)
...
# NVFP4 - dashed line with triangles
line2 = ax1.plot(num_tokens_nvfp4, df_nvfp4["tflops"], color=color_tflops, 
                  marker="^", linestyle="--", label="TFLOPS (NVFP4)")
...
# NVFP4 - dashed line with diamonds
line4 = ax2.plot(num_tokens_nvfp4, df_nvfp4["tb_per_sec"], color=color_tb, 
                  marker="D", linestyle="--", label="TB/sec (NVFP4)")
ax2.tick_params(axis="y", labelcolor=color_tb)

# Add title and legend
plt.title("CUTLASS: Compute and Memory Throughput vs num_tokens")
...
plt.tight_layout()
plt.savefig("throughput_cutlass_plot.png", dpi=150)
plt.show()
